...
def upgrade() -> None:
    ...
    conn = op.get_bind()
    conn.execute(sa.text("DROP TABLE IF EXISTS _alembic_tmp_audit_logs"))
    inspector = sa.inspect(conn)
    if 'pricing_rules' not in inspector.get_table_names():
        ...
        sa.ForeignKeyConstraint(['created_by_id'], ['users.id'], name=op.f('fk_pricing_rules_created_by_id_users'), ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_pricing_rules'))
        )
        op.create_index(op.f('ix_pricing_rules_material'), 'pricing_rules', ['material'], unique=False)
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    if 'pricing_rules' in inspector.get_table_names():
