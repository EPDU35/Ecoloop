...
import { MOBILE_MONEY_PROVIDERS, MobileMoneyProvider } from './MobileMoneyPayment';
...
export default function MobileMoneyPayment({
  ...
  const formatAmount = useCallback((amt: number) => {
    return new Intl.NumberFormat('fr-FR', { style: 'currency', currency }).format(amt);
  }, [currency]);
...
function ProviderSelection({ onSelect }: { onSelect: (id: string) => void }) {
  return (
    <div className="el-mm-providers">
      <h4>Choisissez votre operateur</h3>
...
function PhoneInput({ 
  phone, onChange, formatPhone, onSubmit, loading, disabled 
}: { 
  phone: string; 
...
function PendingPayment({ amount, currency, transactionId, provider, polling }: { 
  amount: number; 
  currency: string; 
  transactionId: string; 
  provider: string;
  ...
  const providerInfo = MOBILE_MONEY_PROVIDERS.find(p => p.id === provider);
  const formatAmount = (amt: number) => new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF' }).format(amt);
  ...
  return (
    <div className="el-mm-pending">
      ...
      <p className="el-mm-amount">Montant: <strong>{new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF' }).format(amount)}</strong></p>
      ...
      <div className="el-pending-steps">
        ...
        <div className="el-step active">
          <span className="el-step-icon">{'.'.repeat(Math.floor(Date.now() / 500) % 3 + 1)}</span>
          <span>Validation par notre equipe en cours</span>
        </div>
...
function SuccessScreen() {
  return (
    <div className="el-mm-success">
      ...
      <p>Votre transaction a ete confirmee avec succes.
...
async function simulatePayment(params: any): Promise<{ success: boolean; transactionId?: string; message?: string; status: string }> {
...
async function checkPaymentStatus(transactionId: string): Promise<'pending' | 'completed' | 'failed' | 'cancelled'> {
