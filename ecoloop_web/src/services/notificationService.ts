...
export interface ChatMessage {
  ...
  timestamp: string;
  read: boolean;
  attachments?: Array<{ url: string; type: 'image' | 'document' }>;
}
...
export function initializeNotifications(userId: string, token: string) {
  ...
  // The service will be initialized with userId and token
  // This is a placeholder - the actual implementation would set these on the service instance
  return service;
}
...
export function getOfflineQueue(): QueuedAction[] {
  try {
    const data = localStorage.getItem('ecoloop_offline_queue');
...
export function addToOfflineQueue(action: Omit<QueuedAction, 'id' | 'timestamp' | 'retries'>) {
  ...
  localStorage.setItem('ecoloop_offline_queue', JSON.stringify(queue));
  ...
  return new Promise(resolve => {
    const processNext = async (index: number) => {
      if (index >= queue.length) {
        localStorage.setItem('ecoloop_offline_queue', '[]');
        resolve();
        return;
      }
