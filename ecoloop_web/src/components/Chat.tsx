...
export default function Chat({ conversationId, onClose: _onClose, className = '' }: ChatProps) {
  ...
  const { messages, sendMessage } = conversation;

  const [messageInput, setMessageInput] = useState('');
  const [sending, setSending] = useState(false);
  const [attachments, setAttachments] = useState<File[]>([]);
  const [_showAttachments, setShowAttachments] = useState(false);
        ...
        <main className={'el-chat-main ' + (activeConversationId ? 'has-conversation' : '')}>
          {!activeConversationId ? (
            <div className="el-chat-empty">
              ...
              <p>Selectionnez une conversation ou creez-en une nouvelle pour commencer a discuter.</p>
