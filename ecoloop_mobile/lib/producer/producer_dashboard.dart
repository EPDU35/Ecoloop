...
class _ProducerDashboardState extends State<ProducerDashboard> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => Provider.of<ProducerProvider>(context, listen: false).fetchData());
  }
  ...
  Widget _home(String name) => SafeArea(
        child: SingleChildScrollView(
          ...
          child: Consumer<ProducerProvider>(builder: (_, pp, __) {
            final r = pp.rewards;
            final kg = r?['total_kg_recycled'] ?? 0;
            final pts = r?['points'] ?? 0;
            final level = r?['level']?.toString().toUpperCase() ?? 'BRONZE';
            final recent = pp.myLots.take(3).toList();
            ...
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Expanded(child: EcoUI.statCard(label: 'kg recyclés', value: kg.toString(), icon: Icons.scale_outlined)),
                const SizedBox(width: 12),
                Expanded(child: EcoUI.statCard(label: 'Collectes', value: pp.myLots.length.toString(), icon: Icons.check_circle_outline)),
              ]),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: EcoUI.statCard(label: 'Points EcoLoop', value: pts.toString(), icon: Icons.monetization_on_outlined)),
                const SizedBox(width: 12),
                Expanded(child: EcoUI.statCard(label: 'Niveau', value: level, icon: Icons.emoji_events_outlined, accent: AppTheme.warn)),
              ]),
              const SizedBox(height: 24),
              if (pp.pricePredictions != null && pp.pricePredictions!.isNotEmpty)
                AnimationHelper.slideUp(
                  child: Container(
                    width: double.infinity,
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Container(padding: const EdgeInsets.all(8),
                          ...
                          child: const Icon(Icons.trending_up, size: 18, color: AppTheme.info)),
                        const SizedBox(width: 10),
                        Text("Prévisions IA", style: GoogleFonts.outfit(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
                        const Spacer(),
                        GestureDetector(
                          onTap: () => Navigator.pushNamed(context, '/price-chart', arguments: 'PLASTIQUE'),
                          child: const Text("Voir tout", style: TextStyle(color: AppTheme.brand, fontSize: 12, fontWeight: FontWeight.w600)),
                        ),
                      ]),
                      const SizedBox(height: 12),
                      SizedBox(
                        ...
                        child: ListView(
                          ...
                          children: pp.pricePredictions!.entries.map((e) {
                            final preds = e.value as List;
                            final latest = preds.isNotEmpty ? preds.first['price'] ?? preds.first['yhat'] ?? 0 : 0;
                            return Padding(
                              padding: const EdgeInsets.only(right: 12),
                              child: GestureDetector(
                                onTap: () => Navigator.pushNamed(context, '/price-chart', arguments: e.key),
                                child: Container(
                                  ...
                                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                                    Text(e.key.substring(0, 4), style: const TextStyle(color: AppTheme.textSoft, fontSize: 12)),
                                    const SizedBox(width: 6),
                                    Text("$latest FCFA", style: const TextStyle(color: AppTheme.brand, fontSize: 12, fontWeight: FontWeight.w700)),
                                  ]),
                                ),
                              ),
                            );
              ...
              if (pp.loading)
                ...
                ...recent.map((lot) => AnimationHelper.slideUp(
                  child: _lotTile(lot),
                )),
            ]);
          }),
  ...
  Widget _lotTile(Map<String, dynamic> lot) => Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text("${lot['weight_kg']} kg ${lot['category']?.toString().toLowerCase() ?? ''}",
                style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
            const SizedBox(height: 2),
            Text(_statusLabel(lot['status']?.toString()), style: TextStyle(fontSize: 12, color: _statusColor(lot['status']?.toString()))),
          ])),
        ]),
      );
        ...
        child: SingleChildScrollView(
          ...
          child: Consumer<ProducerProvider>(builder: (_, pp, __) {
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (pp.loading)
                EcoUI.shimmerLoading()
              else if (pp.myLots.isEmpty)
                ...
                ...pp.myLots.map((lot) => AnimationHelper.slideUp(
                  child: Container(
                    margin: const EdgeInsets.only(bottom: 10),
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(color: AppTheme.inkHigh, borderRadius: BorderRadius.circular(AppTheme.rCard), border: Border.all(color: AppTheme.borderMed)),
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(color: _statusColor(lot['status']?.toString()).withValues(alpha: 0.15), borderRadius: BorderRadius.circular(999)),
                          child: Text(_statusLabel(lot['status']?.toString()), style: TextStyle(color: _statusColor(lot['status']?.toString()), fontSize: 11, fontWeight: FontWeight.w600)),
                        ),
                      ])),
                    ]),
                  ),
  ...
  Widget _recompenses() => SafeArea(
        child: SingleChildScrollView(
          ...
          child: Consumer<ProducerProvider>(builder: (_, pp, __) {
            ...
            final level = r?['level']?.toString().toUpperCase() ?? 'BRONZE';
            final kg = r?['total_kg_recycled'] ?? 0;
            final history = pp.pointsHistory;
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const SizedBox(height: 24),
              ...
              AnimationHelper.scaleIn(
                child: EcoUI.surfaceCard(
                  child: Column(children: [
                    ...
                    Text("$pts points", style: GoogleFonts.outfit(fontSize: 32, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
                    const SizedBox(height: 4),
                    Text("Niveau $level · $kg kg recyclés", style: const TextStyle(color: AppTheme.textSoft, fontSize: 13)),
                  ]),
                ),
              ),
              const SizedBox(height: 20),
              if (history.isNotEmpty) ...[
                Text("Historique", style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
                ...
                ...history.map((h) => Container(
                  ...
                  decoration: BoxDecoration(color: AppTheme.inkHigh, borderRadius: BorderRadius.circular(AppTheme.rCard), border: Border.all(color: AppTheme.borderMed)),
                  child: Row(children: [
                    Icon(((h['points'] ?? 0) as num) >= 0 ? Icons.add_circle : Icons.remove_circle, size: 18,
                        color: ((h['points'] ?? 0) as num) >= 0 ? AppTheme.brand : AppTheme.error),
                    const SizedBox(width: 10),
                    Expanded(child: Text(h['action']?.toString() ?? '', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 13))),
                    Text("${h['points']} pts", style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.w700, color: AppTheme.brand)),
                  ]),
                )),
              ],
            ]);
            ...
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (notifs.isEmpty)
                ...
                ...notifs.map((n) => Container(
                  ...
                  padding: const EdgeInsets.all(14),
                  decoration: BoxDecoration(
                    color: (n['is_read'] == true) ? AppTheme.inkHigh : AppTheme.inkMid,
                    borderRadius: BorderRadius.circular(AppTheme.rCard),
                    border: Border.all(color: (n['is_read'] == true) ? AppTheme.borderMed : AppTheme.brand.withValues(alpha: 0.2)),
                  ),
                  child: Row(children: [
                    Icon((n['is_read'] == true) ? Icons.notifications_none : Icons.notifications_active, size: 20,
                        color: (n['is_read'] == true) ? AppTheme.textFaint : AppTheme.brand),
                    const SizedBox(width: 12),
                    Expanded(child: Text(n['title'] ?? '', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 13))),
                  ]),
                )),
            ]);
          }),
...
class _ProfileContentState extends State<ProfileContent> {
  ...
  Future<void> _save() async {
    ...
    final res = await ApiService.patch('/users/me', {'full_name': _nameC.text.trim(), 'phone': _phoneC.text.trim()});
